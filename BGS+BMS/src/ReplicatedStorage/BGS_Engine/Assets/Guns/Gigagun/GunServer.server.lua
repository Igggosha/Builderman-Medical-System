...
script.Parent.Communications.OnServerEvent:Connect(function(player, action, currentMag)
	if action == "fire" then
		game:GetService("ReplicatedStorage").BGS_Engine.Events.PlaySound:FireAllClients(player, firingSound)
		
		script.Parent.rpg.rocket.Transparency = 1
	...
	elseif action == "reload" then
		...
		if script.Parent.AmmoStored.Value > 0 then
			local co = coroutine.create(function()
				wait(0.2)
				script.Parent.rpg.rocket.Transparency = 0
			end)
			coroutine.resume(co)
		end
