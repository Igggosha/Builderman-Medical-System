...
local firing = false
local firemode: "Safety" | "Semi" | "Auto" = "Safety"
...
function unjam()
	if (not firing) and (not reloading) then
		...
		unjamAnim = player.Character:FindFirstChildWhichIsA("Humanoid"):FindFirstChildWhichIsA("Animator"):LoadAnimation(animobj)
		unjamAnim.Priority = Enum.AnimationPriority.Action2
		unjamAnim:Play()

		VPunjamAnim = arms.Humanoid.Animator:LoadAnimation(animobj)
...
function reload()
	if (not jammed) and (not reloading) then
		gui = player:WaitForChild("PlayerGui"):WaitForChild("BGS_GUI")
		reloading = true
...
function spawnBullet()
	...
	player.PlayerScripts.BGS_Player.Fire:Fire(bulletName, viewgun.Muzzle.Position)
	script.Parent.Communications:FireServer("fire")
	
	
	local soundClone = firingSound:Clone()
	task.spawn(function()
	...
	if player.Character.StanceAndCameraManager.Stance.Value == stances.Standing then
		player.Character.StanceAndCameraManager.Jump:Fire(Vector3.new(math.random(-80,80)/100,math.random(90,150)/100,0))
	elseif player.Character.StanceAndCameraManager.Stance.Value == stances.Crouching then
		player.Character.StanceAndCameraManager.Jump:Fire(Vector3.new(math.random(-40,40)/100,math.random(45,75)/100,0))
	elseif player.Character.StanceAndCameraManager.Stance.Value == stances.Prone then
		player.Character.StanceAndCameraManager.Jump:Fire(Vector3.new(math.random(-20,20)/100,math.random(25,40)/100,0))
	end

	viewportArmsSpring:AddVelocity(-1.5)

	local co = coroutine.create(function()
		TS:Create(viewgun.m249.metal.bolt, TweenInfo.new(0.05, Enum.EasingStyle.Quart), {C1 = CFrame.new(0.4,0,0)}):Play()
		wait(0.05)
		TS:Create(viewgun.m249.metal.bolt, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {C1 = CFrame.new(0,0,0)}):Play()
	end)
...
function unaim()
	...
	TS:Create(GunOffset, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {Value = Vector3.new(0.5,-0.3,-0.6)}):Play()
...
game:GetService("UserInputService").InputBegan:Connect(function(input, ignore)
	...
	if input.UserInputType == Enum.UserInputType.MouseButton1 then
		print(sprintAngle.Value)
		if reloading or math.round(sprintAngle.Value) > 0 then return end
		firing = true
